...
#
package WWW::DialMyCalls::ApiClient;
...
sub _new_instance
{
    ...
    my (%args) = (
        ...
        'base_url' => 'https://api.dialmycalls.com/2.0',
        @_
    );
...
sub _cfg {'WWW::DialMyCalls::Configuration'}
...
sub call_api {
    ...
    $self->{ua}->timeout($self->{http_timeout} || $WWW::DialMyCalls::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $WWW::DialMyCalls::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("WWW::DialMyCalls::Object::$class")->new;
        if (ref $data eq "HASH") {
...
sub get_api_key_with_prefix
{
	...
	my $api_key = $WWW::DialMyCalls::Configuration::api_key->{$key_name};
	
	return unless $api_key;
	
	my $prefix = $WWW::DialMyCalls::Configuration::api_key_prefix->{$key_name};
...
sub update_params_for_auth {
    ...
    foreach my $auth (@$auth_settings) {
        ...
        if (!defined($auth)) {
            ...
            my $api_key = $self->get_api_key_with_prefix('X-Auth-ApiKey');
            if ($api_key) {
                $header_params->{'X-Auth-ApiKey'} = $api_key;
            }
        }
        else {
